...
data Stretch a where
   GapStretch :: (NonEmpty ([a], NonEmpty a)) -> [a] -> Stretch a
   ApStretch :: Stretch (a->b) -> Stretch a -> Stretch b
      deriving (Functor)

minimumLen :: Stretch a -> Int
minimumLen (GapStretch ps ll) = foldr ((+) . length . fst)
                                      (length ll) ps
minimumLen (ApStretch fs xs) = max (minimumLen fs) (minimumLen xs)
